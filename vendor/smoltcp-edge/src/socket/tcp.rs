...
#[cfg_attr(feature = "defmt", derive(defmt::Format))]
/// smoltcp-edge: made `pub` so the public `congestion::Controller`
/// trait's `on_ack` signature is nameable outside the crate.
...
pub struct Socket<'a> {
    ...
    /// smoltcp-edge (T3): external transport-controller slot. When set,
    /// sender bookkeeping runs through the `transport_ext` scoreboard —
    /// per-segment records, SACK consumption, RACK/TLP, DSACK/Eifel —
    /// and the builtin dupack/on_ack path is bypassed. The builtin
    ...
    rx_window_cap: Option<usize>,

    /// smoltcp-edge (T3): sequence range of the last in-order-or-duplicate
    /// payload segment seen below the cumulative edge — emitted as the
...
impl<'a> Socket<'a> {
    ...
    /// smoltcp-edge (T3): install an external transport controller from
    /// `cloud-node-transport`. Once installed, the socket routes all
    /// sender bookkeeping through the `transport_ext` scoreboard —
    /// per-segment records, SACK consumption, RACK-TLP loss detection,
    /// DSACK/Eifel spurious-retransmit undo, pacing and app-limited
    /// marking — and the builtin `congestion_controller` is no longer
    /// consulted for the send window.
    ///
    /// Must be called before the connection is established; installing
    /// mid-flow discards accumulated scoreboard state (none exists yet
    /// — records are pushed on first data transmission).
    pub fn set_transport_controller(
        &mut self,
        cc: alloc::boxed::Box<dyn cloud_node_transport::cc::CongestionController>,
    ) {
        self.ext_transport = Some(transport_ext::ExtTransport::new(cc));
    }

    /// Whether an external transport controller is installed.
    pub fn has_transport_controller(&self) -> bool {
        self.ext_transport.is_some()
    }

    /// smoltcp-edge (T3): controller snapshot for observability
    /// (published into the AF_XDP session `/status` table).
    pub fn transport_snapshot(&self) -> Option<cloud_node_transport::cc::CcSnapshot> {
        self.ext_transport.as_ref().map(|e| e.snapshot())
    }

    /// smoltcp-edge (T3): current pacing ceiling in bytes/s.
    pub fn transport_pacing_rate(&self) -> Option<u64> {
        self.ext_transport
            .as_ref()
            .and_then(|e| e.cc.pacing_rate())
    }

    /// smoltcp-edge (T3): next instant at which the pacing gate allows
    ...
    /// limits the receive window we advertise to `cap` bytes (before
    /// scaling); `None` advertises the full buffer headroom. Lets the
    ...
    ///
    /// smoltcp-edge: `rx_window_cap` shrinks the advertised window for
    /// upper-layer backpressure (T3 rwnd hook) without touching the
    /// buffer itself.
    #[inline]
    fn scaled_window(&self) -> u16 {
        let window = match self.rx_window_cap {
            Some(cap) => self.rx_buffer.window().min(cap),
            None => self.rx_buffer.window(),
        };
    ...
    fn cwnd_remaining(&self) -> usize {
        // smoltcp-edge (T3): under an external controller the send
        // ceiling is `cc.cwnd() - pipe` where pipe excludes SACKed and
        // lost-marked records (Linux tcp_packets_in_flight semantics),
        // unlike `flight_size()` which counts the whole unacked range.
        if let Some(ext) = &self.ext_transport {
            return (ext.cc.cwnd().min(usize::MAX as u64) as usize)
                .saturating_sub(ext.pipe);
        }
    ...
    fn ack_reply(&mut self, ip_repr: &IpRepr, repr: &TcpRepr) -> (IpRepr, TcpRepr<'static>) {
        ...
        if self.remote_has_sack {
            ...
            reply_repr.sack_ranges = [None, None, None];

            let ack = reply_repr.ack_number.unwrap_or(TcpSeqNumber(0));

            // smoltcp-edge (T3): RFC 2883 DSACK — a duplicate segment
            // recorded by `process` goes into the FIRST SACK block so
            // the sender can recognize spurious retransmissions.
            let mut slot = 0;
            if let Some(dup) = self.last_rx_duplicate_range.take() {
                reply_repr.sack_ranges[0] = Some(dup);
                slot = 1;
            }
            ...
            if let Some(last_seg_seq) = self.local_rx_last_seq
                && slot < reply_repr.sack_ranges.len()
            {
                reply_repr.sack_ranges[slot] = self
                    .assembler
                    .iter_data()
            ...
            if slot < reply_repr.sack_ranges.len() && reply_repr.sack_ranges[slot].is_none() {
    ...
    pub(crate) fn process(
        ...
        // smoltcp-edge: the DSACK range set below is valid only for the
        // ACK this segment elicits — clear any leftover first.
        self.last_rx_duplicate_range = None;
        ...
        // smoltcp-edge (T3): RFC 7323 §5.3 PAWS — once timestamps are
        // negotiated, a segment whose TSval is serially older than
        // TS.Recent is a stale replay: drop it. SYN is exempt (RFC 7323
        // §5.4 reboot rule). The 24-day TS.Recent timeout escape is
        // intentionally not implemented — see DIVERGENCE.md.
        if self.state != State::SynSent
            ...
            && (ts.tsval.wrapping_sub(self.last_remote_tsval) as i32) < 0
        {
            net_debug!(
                "PAWS reject: stale tsval {} < TS.Recent {}",
                ts.tsval,
        ...
        let (payload, payload_offset) = match self.state {
            ...
            _ => {
                ...
                if segment_in_window {
                    ...
                    {
                        // smoltcp-edge (T3): a data segment entirely
                        // below the cumulative edge is a duplicate —
                        // carry its range in the reply's first SACK
                        // block as DSACK (RFC 2883 §4).
                        if segment_end <= window_start && segment_start < segment_end {
                            self.last_rx_duplicate_range =
                                Some((segment_start.0 as u32, segment_end.0 as u32));
                        }
            ...
            (State::Listen, TcpControl::Syn) => {
                ...
                if let Some(max_seg_size) = repr.max_seg_size {
                    ...
                    if max_seg_size != 0 {
                        ...
                        // smoltcp-edge (T3): keep the external
                        // controller's MSS quantization in step.
                        if let Some(ext) = &mut self.ext_transport {
                            ext.cc.on_mss_update(self.remote_mss as u64);
                        }
                    }
                }
        ...
        if let Some(ack_number) = repr.ack_number {
            ...
            if self.ext_transport.is_some() {
                // smoltcp-edge (T3): scoreboard-driven ACK processing —
                // dupack/RACK-TLP/DSACK/Eifel live in transport_ext.
                let ext = self.ext_transport.as_mut().unwrap();
                let now_tsval = self.tsval_generator.map(|g| g());
                let _fx = ext.on_segment_ack(
                    cx.now(),
                    ack_number,
                    repr.payload.len(),
                    &repr.sack_ranges,
                    repr.timestamp.map(|t| t.tsecr),
                    now_tsval,
                    is_window_update,
    ...
    fn seq_to_transmit(&self, cx: &mut Context) -> bool {
        ...
        // smoltcp-edge (T3): scoreboard-driven work — a cwnd-admissible
        // lost record or a pending TLP probe must trigger dispatch.
        // (Not gated by the pacing deadline: retransmits are exempt.)
        if let Some(ext) = &self.ext_transport {
            if ext.tlp_probe_pending() {
                return true;
            }
            if ext
                .next_lost(self.remote_mss, self.local_seq_no)
                .is_some()
                && (ext.pipe as u64) < ext.cc.cwnd()
            {
                return true;
            }
        }
        ...
        F: FnOnce(&mut Context, PacketMeta, (IpRepr, TcpRepr)) -> Result<(), E>,
    {
        ...
        // smoltcp-edge (T3): external-controller timers — the RACK loss
        // deadline and the TLP probe timeout are checked every dispatch.
        if let Some(ext) = &mut self.ext_transport {
            ext.timer_checks(cx.now());
        }
        ...
        if self.timed_out(cx.now()) {
            ...
            if let Timer::Retransmit { .. } = self.timer {
                ...
                if let Some(ext) = &mut self.ext_transport {
                    ...
                    // every outstanding record lost; the controller is
                    // handed a persistent-loss event (it runs its own
                    // RTO collapse inside on_loss_event). No
                    // remote_last_seq rewind — retransmission picks
                    // from the scoreboard.
                    ext.mark_all_lost(cx.now());
                } else {
                ...
                // smoltcp-edge: the ext dupack path never arms this
                // timer; if it fired anyway, the scoreboard already
                // handles recovery — do not also rewind/flag.
            }
        ...
        // smoltcp-edge: whether the emitted payload is a scoreboard
        // retransmission (Karn applies; pacing gate exempt).
        let mut ext_retransmit = false;
        ...
        match self.state {
            ...
            | State::LastAck => {
                ...
                let offset = if self.pending_fast_retransmit {
                    ...
                    0
                } else if let Some((seq, len)) = self.ext_transport.as_ref().and_then(|ext| {
                    // smoltcp-edge (T3): scoreboard retransmission takes
                    // precedence over new data, gated by the controller's
                    // congestion window (retx re-enters the pipe).
                    ext.next_lost(effective_mss, self.local_seq_no).filter(|_| {
                        (ext.pipe as u64) < ext.cc.cwnd()
                    })
                }) {
                    let off = seq - self.local_seq_no;
                    repr.seq_number = seq;
                    repr.payload = self.tx_buffer.get_allocated(off, len);
                    ext_retransmit = true;
                    off
                } else {
                    ...
                    } else if let Some(ext) = &self.ext_transport {
                        // smoltcp-edge (T3): ext new-data send — cwnd via
                        // the scoreboard pipe, pacing deadline gate, and
                        // a pending TLP probe may push one segment even
                        // while cwnd/pacing would otherwise hold.
                        if ext.tlp_probe_pending() {
                            win_limit.min(device_limit).min(effective_mss)
                        } else if !ext.pacing_allows(cx.now()) {
                            0
                        } else {
                    ...
                    let mut offset = self.flight_size();
                    repr.payload = self.tx_buffer.get_allocated(offset, size);

                    // smoltcp-edge: TLP fallback — no new data available
                    // (or nothing fits), so the probe retransmits the
                    // tail unacked record instead.
                    if !is_zero_window_probe
                        && repr.payload.is_empty()
                        && let Some((seq, len)) = self.ext_transport.as_ref().and_then(|ext| {
                            if ext.tlp_probe_pending() {
                                ext.tail_unacked(effective_mss, self.local_seq_no)
                            } else {
                                None
                            }
                        })
                    {
                        offset = seq - self.local_seq_no;
                        repr.seq_number = seq;
                        repr.payload = self.tx_buffer.get_allocated(offset, len);
                        ext_retransmit = true;
                    }
        ...
        // which occupies sequence space like any new-data send. Runs
        // before the zero-window/keep-alive early returns below.
        if let Some(ext) = &mut self.ext_transport
            && !repr.payload.is_empty()
            && !is_keep_alive
        {
            let payload_len = repr.payload.len();
            let tsval = repr.timestamp.map(|t| t.tsval).unwrap_or(0);
            ext.note_send_resumed(cx.now());
            ext.note_sent(cx.now(), repr.seq_number, payload_len, tsval, ext_retransmit);
            if ext_retransmit {
                // Retransmits bypass the pacing gate.
            } else {
                ext.pace_after_send(cx.now(), payload_len);
            }
            if ext.tlp_probe_pending() {
                ext.clear_tlp_probe();
            }
            // App-limited: the write queue is fully in flight after this
            // segment (Linux marks tp->app_limited on drain).
            if !ext_retransmit
                && repr.seq_number + payload_len
                    == self.local_seq_no + self.tx_buffer.len()
            {
                ext.mark_app_limited();
            }
        }
    ...
    pub(crate) fn poll_at(&self, cx: &mut Context) -> PollAt {
        ...
        if self.tuple.is_none() {
            ...
            // smoltcp-edge pacing gate holds new data until its deadline
            // (retransmit/probe work is exempt and stays Now).
            match self.ext_transport.as_ref() {
                Some(ext)
                    if ext.next_lost(self.remote_mss, self.local_seq_no).is_none()
                        ...
                        && ext
                            ...
                            .unwrap_or(false) =>
                {
                    PollAt::Time(ext.next_send_due.unwrap())
                }
                _ => PollAt::Now,
            }
        } else if self.window_to_update() {
            ...
            // smoltcp-edge: RACK/TLP deadlines ride the same poll chain.
            let ext_poll_at = self
                .ext_transport
                .as_ref()
                .and_then(|e| e.next_timer())
                .map(PollAt::Time)
...
mod test {
    ...
    // =========================================================================================//
    // Tests for the smoltcp-edge external transport controller (T3).
    // =========================================================================================//

    #[test]
    fn test_external_transport_controller() {
        use cloud_node_transport::cc::reference::CubicRef;

        let mut s = socket_established_with_buffer_sizes(8192, 64);
        s.remote_win_len = 65535;
        s.remote_mss = 1024;
        assert!(!s.has_transport_controller());
        assert_eq!(s.transport_snapshot(), None);

        s.set_transport_controller(Box::new(CubicRef::new(1024)));
        assert!(s.has_transport_controller());

        // CubicRef initial cwnd = 4*MSS = 4096.
        let snap = s.transport_snapshot().unwrap();
        assert_eq!(snap.algo, "cubic_ref");
        assert_eq!(snap.cwnd_bytes, 4 * 1024);

        let data = [b'x'; 8192];
        s.send_slice(&data[..]).unwrap();

        // Only 4 segments may be in flight: cwnd gates the ext path.
        recv!(s, time 0, [
            TcpRepr {
                seq_number: LOCAL_SEQ + 1,
                ack_number: Some(REMOTE_SEQ + 1),
        ...
        // ACK the first segment: slow start grows cwnd by the acked
        // bytes (RFC 3465 ABC cap 2*MSS -> +1024), freeing space for
        // two more segments (2048 in flight allowed).
        send!(
            s,
            time 50,
            TcpRepr {
        ...
        let snap = s.transport_snapshot().unwrap();
        assert_eq!(snap.cwnd_bytes, 5 * 1024);
        // The ACK produced an RTT sample on the scoreboard.
        assert!(snap.min_rtt.is_some());

        // cwnd admits both, but pacing spaces them ~10ms apart
        // (5120 B / 50 ms srtt = 102400 B/s -> 1024 B per 10 ms).
    ...
    #[test]
    fn test_external_transport_loss_event_shrinks_cwnd() {
        use cloud_node_transport::cc::reference::CubicRef;
        ...
        // Fill cwnd: 4 segments in flight.
        recv!(s, time 0, [
            TcpRepr { seq_number: LOCAL_SEQ + 1, ack_number: Some(REMOTE_SEQ + 1), payload: &data[..1024], ..RECV_TEMPL },
            TcpRepr { seq_number: LOCAL_SEQ + 1 + 1024, ack_number: Some(REMOTE_SEQ + 1), payload: &data[1024..2048], ..RECV_TEMPL },
            TcpRepr { seq_number: LOCAL_SEQ + 1 + 2048, ack_number: Some(REMOTE_SEQ + 1), payload: &data[2048..3072], ..RECV_TEMPL },
            TcpRepr { seq_number: LOCAL_SEQ + 1 + 3072, ack_number: Some(REMOTE_SEQ + 1), payload: &data[3072..4096], ..RECV_TEMPL }
        ]);

        // Three duplicate ACKs SACK-ing [1024..4096] while the first
        // segment is missing trip the frontier mark: every unconfirmed
        // record below the SACK edge is marked lost and the loss event
        // reaches the controller.
        let mut sack_ranges = [None, None, None];
        sack_ranges[0] = Some((
            (LOCAL_SEQ + 1 + 1024).0 as u32,
            (LOCAL_SEQ + 1 + 4096).0 as u32,
        ));
