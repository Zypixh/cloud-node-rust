...
impl Repr {
    ...
    pub fn new(
        ...
        match (src_addr, dst_addr) {
            ...
            (Address::Ipv4(src_addr), Address::Ipv4(dst_addr)) => Self::Ipv4(Ipv4Repr {
                ...
                next_header,
                payload_len,
                // smoltcp-edge: ECT marking is a T7 decision; until then
                // locally originated packets carry Not-ECT.
                ecn: 0,
                hop_limit,
            }),
            #[cfg(feature = "proto-ipv6")]
    ...
    /// smoltcp-edge (T3): the ECN codepoint carried by this repr
    /// (0..=3); `0` = Not-ECT.
    pub const fn ecn(&self) -> u8 {
        match *self {
            #[cfg(feature = "proto-ipv4")]
            Repr::Ipv4(repr) => repr.ecn,
            #[cfg(feature = "proto-ipv6")]
            Repr::Ipv6(repr) => repr.ecn,
        }
    }
