...
mod tests {
    ...
    fn test_fuzz_differential_case_skv() {
        ...
        assert_eq!(
            is_sqli, false,
            "Should match C implementation - fingerprint: {}",
            fp
        );
        ...
        for (input_str, expected_value) in test_cases {
            ...
            if let Some(token) = tokenizer.next_token() {
                assert_eq!(
                    token.token_type,
                    TokenType::Variable,
                    "Token type should be Variable for input '{}'",
                    input_str
                );
                assert_eq!(
                    token.value_as_str(),
                    expected_value,
                    "Token value should preserve @ symbols for input '{}'",
                    input_str
                );
                assert_eq!(
                    token.pos, 0,
                    "Token position should start at 0 for input '{}'",
                    input_str
                );
            } else {
    ...
    fn debug_semicolon_issue() {
        ...
        let mut state = SqliState::from_string(
            input,
            SqliFlags::new(SqliFlags::FLAG_QUOTE_NONE.0 | SqliFlags::FLAG_SQL_ANSI.0),
        );
        ...
        assert!(
            fingerprint.as_str().contains(";"),
            "Fingerprint should contain semicolon"
        );
    }
    ...
    fn test_select_float_version() {
        ...
        assert_eq!(
            fingerprint.as_str(),
            "Ev;",
            "Fingerprint should be Ev; for 'SELECT float @@version;'"
        );
    }
    ...
    fn test_fuzz_input_with_quote_double() {
        ...
        let mut state = SqliState::new(
            input,
            SqliFlags::new(SqliFlags::FLAG_QUOTE_DOUBLE.0 | SqliFlags::FLAG_SQL_MYSQL.0),
        );
        ...
        assert_eq!(
            fingerprint.as_str(),
            "s",
            "Expected fingerprint 's' but got '{}'",
            fingerprint.as_str()
        );
    }
    ...
    fn test_fuzz_input_with_detect() {
        ...
        println!(
            "  Is blacklisted: {}",
            blacklist::is_blacklisted(fp1.as_str())
        );

        let mut state2 = SqliState::new(
            input,
            SqliFlags::new(SqliFlags::FLAG_QUOTE_DOUBLE.0 | SqliFlags::FLAG_SQL_MYSQL.0),
        );
        ...
        println!(
            "  Is blacklisted: {}",
            blacklist::is_blacklisted(fp2.as_str())
        );
    ...
    fn test_quote_context_differential_fuzzing_case() {
        ...
        assert_eq!(
            result.is_injection(),
            true,
            "Should detect SQLi like C implementation"
        );
        assert_eq!(
            result.fingerprint.as_ref().map(|f| f.as_str()),
            Some("sos"),
            "Should generate 'sos' fingerprint like C implementation"
        );

        // Test the internal state with different quote contexts
        let mut state_ansi = SqliState::new(
            &input,
            SqliFlags::new(SqliFlags::FLAG_QUOTE_NONE.0 | SqliFlags::FLAG_SQL_ANSI.0),
        );
        ...
        assert_eq!(
            fp_ansi.as_str(),
            "ons",
            "ANSI context should produce 'ons' fingerprint"
        );

        let mut state_double_quote = SqliState::new(
            &input,
            SqliFlags::new(SqliFlags::FLAG_QUOTE_DOUBLE.0 | SqliFlags::FLAG_SQL_ANSI.0),
        );
        let fp_double = state_double_quote.fingerprint();
        assert_eq!(
            fp_double.as_str(),
            "sos",
            "Double quote context should produce 'sos' fingerprint"
        );

        // The double quote context version should be detected as SQLi
        assert!(
            state_double_quote.check_is_sqli(&fp_double),
            "Double quote context should detect as SQLi"
        );
    }
    ...
    fn test_evil_token_fingerprint_reset() {
        ...
        assert_eq!(
            state.tokens.len(),
            1,
            "Should have exactly 1 token after Evil reset"
        );
        assert_eq!(
            state.tokens[0].token_type,
            TokenType::Evil,
            "Single remaining token should be Evil type"
        );
        assert_eq!(
            state.tokens[0].value_as_str(),
            "X",
    ...
    fn test_fuzz_crash_c_behavior() {
        ...
        assert!(
            is_sqli,
            "Should detect as SQL injection to match C behavior"
        );
    ...
    fn test_mysql_reparse_logic() {
        ...
        println!(
            ...
            token
                .as_ref()
                .map(|t| (t.token_type.clone(), t.value_as_str()))
        );
        println!(
            "  Tokenizer stats_comment_ddx: {}",
            tokenizer.stats_comment_ddx
        );
        println!(
            "  Tokenizer stats_comment_ddw: {}",
            tokenizer.stats_comment_ddw
        );
        ...
        assert!(
            state.stats_comment_ddx > 0,
            "stats_comment_ddx should be set for '--' not followed by whitespace"
        );
        ...
        assert!(
            state.tokens.len() > 1,
            "MySQL mode should produce multiple tokens from '--1-@a#*\\x03'"
        );
    ...
    fn test_php_backquote_comment_fuzz_case() {
        ...
        for (i, token) in state.tokens.iter().enumerate() {
            println!(
                "  Token {}: type={:?}, val='{}', str_open=0x{:02x}, len={}, str_close=0x{:02x}",
                i,
                token.token_type,
                token.value_as_str(),
                token.str_open,
        ...
        let fuzz_input = &[
            0xd8, 0x24, 0xff, 0x2a, 0x22, 0x22, 0x1c, 0x22, 0x22, 0x32, 0x60,
        ];
    ...
    fn test_original_fuzz_differential_input() {
        ...
        let flags = SqliFlags::new(SqliFlags::FLAG_QUOTE_DOUBLE.0 | SqliFlags::FLAG_SQL_MYSQL.0);
        ...
        assert!(
            !is_sqli_rust,
            "Should not detect as SQL injection (matching C behavior)"
        );
        ...
        assert!(
            !is_sqli_rust,
            "Should not detect as SQL injection with FLAG_QUOTE_NONE"
        );
    }
    ...
    fn test_fuzz_differential_whitelist_bug() {
        ...
        assert_eq!(
            fingerprint, "1c",
            "Should produce '1c' fingerprint (number + comment)"
        );
        assert_eq!(
            state.stats_tokens, 2,
            "Should have stats_tokens = 2 (single tokenization pass)"
        );
        assert_eq!(
            is_sqli, false,
            "Should return false due to C bug compatibility"
        );
        ...
        assert_eq!(
            state.tokens[0].value_as_str(),
            "8",
            "First token value should be '8'"
        );
        assert_eq!(
            state.tokens[0].pos, 1,
            "First token should start at position 1 (after \\x1b)"
        );
        assert_eq!(state.tokens[0].len, 1, "First token should have length 1");

        assert_eq!(
            state.tokens[1].token_type,
            TokenType::Comment,
            "Second token should be Comment"
        );
        assert_eq!(
            state.tokens[1].value_as_str(),
            "--",
            "Second token value should be '--'"
        );
        assert_eq!(
            state.tokens[1].pos, 2,
            "Second token should start at position 2"
        );
        assert_eq!(state.tokens[1].len, 2, "Second token should have length 2");

        println!(
            "✅ C bug compatibility verified - input correctly returns false despite being SQLi"
        );
    }
    ...
    fn test_fuzz_differential_evil_token_fix() {
        ...
        let input = vec![
            ...
            255, // 11 more bytes including 0xFF
        ...
        assert_eq!(
            fingerprint.as_str(),
            "X",
            "Fingerprint should be 'X' when Evil tokens are present"
        );
        ...
        assert_eq!(
            state.tokens.len(),
            1,
            "Should have exactly 1 token after Evil fingerprint processing"
        );
        assert_eq!(
            state.tokens[0].token_type,
            TokenType::Evil,
            "Single token should be Evil type"
        );
        assert_eq!(
            state.tokens[0].value_as_str(),
            "X",
        ...
        println!(
            "✅ Fuzz differential fixed - Evil tokens now contribute to SQL injection detection"
        );
    }
    ...
    fn test_fuzz_differential_crash_dd7a369a() {
        ...
        let input = [
            92, 42, 92, 92, 65, 115, 64, 92, 92, 92, 208, 33, 33, 208, 33, 92, 33, 52, 52, 33, 52,
            255, 255, 255, 33, 255, 255, 49, 106, 255, 33, 64, 43, 42, 33, 43,
        ];
    ...
    fn test_fuzz_differential_input_1a0_2d_2d_02() {
        ...
        println!(
            "Initial flags after FLAG_NONE conversion: {:?}",
            state1.flags
        );
        ...
        println!(
            "check_is_sqli('1c'): {}",
            state1.check_is_sqli(&fingerprint1)
        );
        ...
        if state1.tokens.len() >= 2 {
            println!(
                "  token[0]: {:?} '{}' pos={} len={}",
                state1.tokens[0].token_type,
                state1.tokens[0].value_as_str(),
                state1.tokens[0].pos,
                state1.tokens[0].len
            );
            println!(
                "  token[1]: {:?} '{}' pos={} len={}",
                state1.tokens[1].token_type,
                state1.tokens[1].value_as_str(),
                state1.tokens[1].pos,
                state1.tokens[1].len
            );
        }
        ...
        if state1.reparse_as_mysql() {
            ...
            let mysql_flags =
                (original_flags.0 & !SqliFlags::FLAG_SQL_ANSI.0) | SqliFlags::FLAG_SQL_MYSQL.0;
    ...
    fn test_fuzz_differential_percent_signs() {
        ...
        // Expected: Rust should return the same as C (C returns false, Rust currently returns true)
        let input = &[
            37u8, 37, 37, 37, 37, 37, 37, 37, 37, 37, 37, 37, 37, 37, 37, 37, 37, 37, 37, 37, 37,
            37, 35, 42, 35, 254, 34, 14, 37, 37, 113, 93, 47, 47, 42, 33, 241, 39, 35, 38, 126, 35,
            97, 47,
        ];
    ...
    fn test_fuzz_differential_quote_newline_quote() {
        ...
        // Expected: Rust should return the same as C (C returns false, Rust currently returns true)
        let input = &[
            113u8, 39, 255, 39, 255, 255, 255, 255, 255, 255, 255, 255, 39, 10, 43, 39, 39, 35,
        ];
        ...
        println!(
            "  C fingerprint 'snsos' blacklisted: {}",
            blacklist::is_blacklisted("snsos")
        );
        println!(
            "  Rust fingerprint 'sosc' blacklisted: {}",
            blacklist::is_blacklisted("sosc")
        );
    ...
    fn test_fuzz_differential_backslash_quote_pattern() {
        ...
        let input = &[
            92u8, 92, 92, 92, 92, 92, 92, 92, 92, 34, 92, 92, 92, 92, 92, 92, 92, 92, 34, 93, 34,
            47, 42, 33, 92, 34, 94, 42,
        ];
