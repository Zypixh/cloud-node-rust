...
mod tests {
    ...
    #[test]
    fn test_crash_input() {
        use crate::xss::{AttributeType, Html5Flags, Html5State, TokenType, XssDetector};
        ...
        println!(
            "Real detect_xss result: {:?} (is_injection: {})",
            real_result,
            real_result.is_injection()
        );
    ...
    fn debug_is_xss_with_trace(input: &[u8], flags: crate::xss::Html5Flags) -> bool {
        use crate::xss::{AttributeType, Html5State, TokenType, XssDetector};
        ...
        while html5.next() && token_count < 20 {
            ...
            if html5.token_len > 0 && html5.token_len <= html5.token_start.len() {
                ...
                println!(
                    "  Content: {:?} (raw: {:?})",
                    String::from_utf8_lossy(token_slice),
                    token_slice
                );
            }
            ...
            match html5.token_type {
                ...
                TokenType::TagNameOpen => {
                    if html5.token_len > 0 && html5.token_len <= html5.token_start.len() {
                        ...
                        println!(
                            "  -> Checking tag name: {:?}",
                            String::from_utf8_lossy(tag_slice)
                        );
                        ...
                        let is_blacklisted = tag_slice.len() >= 3 && {
                            ...
                            let blacklist = [
                                ...
                                "META", "NOSCRIPT", "OBJECT", "SCRIPT", "STYLE", "VMLFRAME", "XML",
                                "XSS",
                            ];
                ...
                TokenType::AttrName => {
                    if html5.token_len > 0 && html5.token_len <= html5.token_start.len() {
                        ...
                        println!(
                            "  -> Checking attribute: {:?}",
                            String::from_utf8_lossy(attr_slice)
                        );
                ...
                TokenType::TagComment => {
                    if html5.token_len > 0 && html5.token_len <= html5.token_start.len() {
                        ...
                        println!(
                            "  -> Checking comment: {:?}",
                            String::from_utf8_lossy(comment_slice)
                        );
