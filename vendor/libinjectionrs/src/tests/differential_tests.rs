...
pub fn run_full_differential_tests() -> DifferentialTestResult {
    ...
    for category in test_categories {
        ...
        for test_file in test_files.into_iter().take(10) {
            ...
            println!(
                "    📊 {}/{} matches ({:.1}%)",
                file_matches, file_tests, match_rate
            );
    ...
    if total_tests > 0 {
        println!(
            "Total matches: {}/{} ({:.1}%)",
            total_matches, total_tests, overall_rate
        );
        ...
        if overall_rate >= 95.0 {
            ...
            println!(
                "\n⚠️  Acceptable functionality ({:.1}%) - some issues",
                overall_rate
            );
        } else {
            println!(
                "\n❌ Poor functionality ({:.1}%) - needs investigation",
                overall_rate
            );
        }
    } else {
