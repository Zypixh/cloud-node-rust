    ...
    while index < bytes.len() {
        if bytes[index] == b'$' {
            ...
            if digits_end > digits_start
                && let Ok(capture_index) = replacement[digits_start..digits_end].parse::<usize>()
            {
...
fn expand_go_regex_replacement(
    ...
    while index < bytes.len() {
        if bytes[index] == b'$'
            && index + 1 < bytes.len()
            && bytes[index + 1] == b'{'
            && let Some(end_offset) = replacement[index + 2..].find('}')
        {
...
pub fn evaluate_host_redirects(
    ...
    for redirect in redirects {
        ...
        {
            if domain_matches(std::slice::from_ref(before), host) {
                let location = if redirect.keep_request_uri {
