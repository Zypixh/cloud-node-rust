            ...
            if let Some(result) = matcher_plus::match_group(group, session, request_body) {
                if let Some(set) = result.set {
                    if let Some(mut matched) = perform_actions(&set.actions) {
                        ...
                        // Flow Control: GO_SET
                        if let Some(_next_sid) = matched.next_set_id {
                             // Complex set jumping is usually handled within the group iteration
                        }
                ...
                if result.matched {
                    return Some(default_block_action(policy.id, group.id));
                }
            }
            current_group_idx += 1;
        }
