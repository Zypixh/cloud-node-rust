...
fn cell(delay_ms: u64, uniform_loss: f64, rate_mbps: u64) {
    ...
    for (label, mut cc) in [
        ("edgecc", Box::new(EdgeCc::new(cfg.mss, Tier::T1, None, false)) as Box<dyn CongestionController>),
        ...
        println!(
            ...
            res.completed, res.fct_us as f64 / 1e6, res.rto_events, res.retransmits, snap.mode, snap.reason_code,
        );
    }
}
...
fn main() {
    for (name, rtt, loss, mbps) in [
        ("rtt80 loss0% 100M", 80u64, 0.00, 100u64),
        ("rtt80 loss1% 100M", 80, 0.01, 100),
        ("rtt80 loss2% 100M", 80, 0.02, 100),
        ("rtt80 loss5% 100M", 80, 0.05, 100),
        ("rtt80 loss1% 20M", 80, 0.01, 20),
        ("rtt150 loss2% 50M", 150, 0.02, 50),
    ] {
        println!("== {name}");
        cell(rtt, loss, mbps);
    }
