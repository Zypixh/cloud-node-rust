...
2026-09-14 续接约束：用户手动运行 Devin，按 [VPS 续接计划](devin-vps-continuation.md) 自主分阶段实施。本机仅编辑与传输，禁止在 Mac 或本机 VM 编译/测试；实际验证迁至两台获授权 VPS。该续接计划覆盖本文旧的单任务调度建议和历史构建地点，不改变架构不变量与验收门槛。下文 Cargo/测试命令仅在远端隔离环境执行。
