...
## Current XDP transport review — 2026-09-17, baseline a094602

Planning/static review only: no source changes, builds, tests, VPS execution, or deployment in this review. Current XDP transport scope and the **single continuous-execution prompt** are in [transport plan v3.2, section 8](xdp-transport-next-steps-2026-09-15.md). That entry replaces the old per-stage prompts below for transport work, not the entire EN-00..33 productionization scope.

- T1/T2/T3 and T4 implementation have historical EN-18..22/24 evidence. EN-24 reports dual-stack traffic, outbound dial, IPv4 PTB, guard and Cubic event-comparison results; these were not independently rerun here.
- F1 implemented and verified on kernel 6.1 (EN-24 appendix A): lease-adoption handover keeps AF_XDP workers/sessions alive across compatible reloads; on <6.4 kernels without XDP BPF_LINK_UPDATE the handover uses dispatch-table swap gated by entry-program tag equality (bytecode-identical dispatcher required, otherwise the reload is rejected explicitly). Live-traffic re-test: 74KB echo through a mid-session reload with zero RST and a fully ready successor generation. XDP_XSKS is now pinned so dispatch workers share socket registrations. Remaining limits recorded in EN-24.
- T4-9 and T5–T10 ~~remain TODO~~ (superseded by next bullet: T5–T9 VERIFIED per EN-25; T10 ranking + T4-9 coverage accounting remain open). Execute them continuously under one assignment; internal checkpoints do not require another user prompt. T3 packet-script coverage still requires explicit coverage accounting. IPv6 PTB injection is now done live (EN-24 appendix B): ICMPv6 type-2 → eBPF out-CT claim (`outCtIcmp`) → `apply_pmtu` → wire MSS convergence 1400→1220+180, plus unit coverage for non-PTB no-op and unknown-flow miss.
- T5–T9 implemented and verified on build-90 (EN-25): EdgeCC decision engine + reference controllers (ce9276a), smoltcp-edge AccECN RFC 9768 wire+socket (ec62097), userspace wiring — EdgeCC assembly, ECN metadata chain, hierarchical scheduler/DRR/CoDel, adaptive rx buffering, QUIC EdgeCC adapter (25145bb). Acceptance: 781 lib + 96 transport + 691 smoltcp-edge tests green; netns full-protocol smoke pass; deterministic matrix 6,948 rows archived with ablation deltas. Honest limitations recorded in EN-25 §3.3/§6: EdgeCC startup overshoot on high-RTT shallow-buffer paths, weak same-path concurrency fairness, p_rand/plateau ablations indistinguishable within budget. Sim event/metric fixes in 8f91503/9207e80 (same-tick RTO spin, retx pacing escape, SACK-time RTT sampling).
- Additional static findings to reproduce: unconditional receive-checksum bypass in smoltcp AF_XDP capabilities, TX-ring-full drops/ignored cross-interface send errors, and zero-envelope refill liveness. Full Quinn sampling/pacing and direct XDP_TX shaping coverage need feasibility/contract verification; extending approved patch scope needs approval.
- Development/build host: devin-build-90, /root/cloud-node-rust. Local Mac remains edit/static-only. Native/copy evidence is not zero-copy or hardware multi-queue certification; missing hardware/public ECN samples remain external acceptance blockers.
- No new degradation approved; no automatic production-default change, push or deployment. Record tests by actual commit/environment and retain all unresolved requirements. The 2026-09-15 entries below are historical and do not override this transport review.

## Historical review override — 2026-09-15, baseline e3fb282
