...
- **TCP 纯 L4 透传走 XDP NAT**（性能优先，不用 sockmap）：4-tuple DNAT+SNAT，逐包重写+校验和，`XDP_TX` 直发——不过内核 TCP 协议栈，线速。透传不改 payload，seq/ack 无需 delta 重写；回包路径依赖 backend 回流量经本节点（与 UDP NAT 同前提）。
  ...
  - **eBPF 工程约束（已在 OrbStack VM kernel 7.0 实测验证）**：512B 栈上限要求大 key/value 结构放 `XDP_NAT_SCRATCH` per-CPU scratch（且字段级写入，结构体字面量会走栈+memcpy）；四个 NAT 处理函数 `#[inline(never)]` + **tail call 分派**（`XDP_DISPATCH` ProgramArray slot0=`xdp_nat_dispatch` 独立 XDP 子程序，子程序自解析包头——跨程序传偏移会丢 verifier 边界跟踪）。tail-call 空槽 → 显式回落 inline `maybe_redirect`。bpftool 验证：两程序 verifier 全过，generic XDP attach eth0 + ping 实测正常。**待真网卡验证**：XDP_TX 直发路径、邻居解析、驱动模式（virtio netmap）。
  - **SNAT 模式（云 anti-spoof 适配，已实测定因）**：真机验证发现主流云平台（本次实测 virtio + vSwitch）按端口绑定源 IP 做 egress anti-spoof——保留客户端 IP 的 DNAT 直发帧在宿主机 vSwitch 层被丢（`tx_queue_*_xdp_tx` 计数成功但对端收不到；同内容帧走内核 AF_PACKET 路径同样被丢，证明与 XDP 无关）。对策：`udpForwards[]`/`tcpForwards[]` 新增 `snat: true` 选项——eBPF 将源改写为 `(listen_addr, 节点分配端口)`，端口空间 `40000..60999` 由 `snat_alloc` 经确定性 hash + `BPF_NOEXIST` 有界探测（≤8 次，跳过与已配置 listen tuple 冲突的端口）认领，反向绑定写入 `XDP_SNAT_REV`（listen+port+proto → 完整客户端四元组+MAC）。回包经反向绑定还原客户端 tuple 后 `XDP_TX`；绑定校验源地址/端口必须匹配登记的 backend 元组，不匹配则不视为回包。已存在的非 SNAT 流保持 DNAT 至超时（不在流中途换元组）；端口耗尽 → `snat_alloc_fail` 计数 + 显式回落用户态路径；孤儿绑定由 sweeper 随 CT 过期清扫（按 value 内四元组重建 CT key 判活）。计费/计数沿用 `XDP_FLOW_ACCT` + `snat_bound`/`snat_alloc_fail`/`snat_reply_tx`。代价：backend 看不到真实客户端 IP（如需要可叠加 PROXY 协议，属后续项）。
...
- **SNI 快速路径**（已实现为防御性 blocklist 快路径，非 L4 直转）：原计划"SNI 完整则按路由 map 做 L4 直转"在节点终结型代理下不成立——SYN 握手必须先有后端，而 SNI 在握手后的首个数据段才出现；SYN PASS 给内核后再 DNAT，后端收到的是从未握手的连接 → RST。实际实现：eBPF 解析 TCP 首数据段的 TLS ClientHello，命中 `XDP_SNI_BLOCK`（FNV-1a(lower(SNI)) hash map）→ `XDP_DROP` 线速防御；未命中/不完整 → PASS 进用户态权威路径。**明确的快路径边界（非降级，均有 `sni_incomplete` 计数）**：仅解析第一个扩展（server_name 在绝大多数 ClientHello 中为首扩展）；域名 > `XDP_SNI_MAX_LEN`(16) 走用户态；长度字段按 TLS 语义钳制（sid≤32/cs≤512/comp≤8/ext_total≤2048/ext_len≤255）。`xdp_sni_dispatch` 为 `XDP_DISPATCH` slot1 独立子程序（独立栈/指令预算），NAT 链在 SNI 之后（slot0）。userspace：`XdpConfig.sniBlocklist` → `sync_sni_blocklist`（清旧值后写入）；对象缺 map/dispatch → 显式告警；计数 `sni_blocked`/`sni_incomplete` 已入 `XdpStatusSnapshot`。bpftool 验证（OrbStack kernel 7.0）：三程序（cloud_node_xdp/xdp_nat_dispatch/xdp_sni_dispatch）verifier 全过、attach eth0 generic + ping 实测正常。verifier 对抗要点：`black_box(off)` 阻止 LLVM 把 `data+off` 拆成两条推导链导致 range 标记不传递；字节读全部 `.map(u64::from)` 消除 8-bit spill 与指针槽位冲突。
