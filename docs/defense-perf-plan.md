...
- **QUIC DCID 路由**：透传 → DCID 查 map 直转后端；终结在本机的 H3 → **用户态共享 CidRoutes**（跨全部 queue reactor 可见）按 DCID 路由到正确 session，解决 QUIC 多核扩展与连接迁移。
  - 实现状态（已修正）：**`XDP_QUIC_DCID` 已移除**。原方案按 DCID pin 目标 XSK index 做跨队列 redirect——但 XSKMAP redirect 要求目标 XSK 绑定在当前 ingress (netdev, rx_queue)，RSS 把包哈希到非 CID 归属队列时该 redirect 被内核直接丢包，方案本身不成立。现行为：eBPF 只做 queue-local 投递（`XDP_XSK_INDEX` 当前队列），跨队列 CID 归路由由共享用户态 `quic_udp_demux`/`CidRoutes` 完成（short header 本就无编码长度，只能走此路径）。
- **SNI 快路径已移除**：内核侧 SNI 名单（`XDP_SNI_BLOCK` / `xdp_sni_dispatch` / `sniBlocklist` 配置）已整体删除。域名级判断属于 L7 职责，由用户态 TLS acceptor 的证书选择器隐含完成——未配置域名的 ClientHello 自然被拒绝，不需要第二张名单（与 Cloudflare 同模型：eBPF 只做包级缓解，域名下线=移除 server 配置）。顺带收益：每个 TCP 包省一次 tail-call + SNI 前置解析开销。`XDP_DISPATCH` slot1 保留为空槽。
