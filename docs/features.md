...
缓存命中路径会优先读取内存索引，减少 RocksDB 热路径读写。小对象 L2 命中返回内存 handler，确保热对象可以晋升 Fast L1；大对象 L2 命中按文件块输出，压缩对象可流式 zstd 解压，避免整文件读入内存。缓存写入使用临时文件和 rename，降低并发写同一对象造成的数据损坏风险。
