...
缓存命中路径会优先读取内存索引，避免 Mace I/O 进入热路径。小对象 L2 命中返回内存 handler，确保热对象可以晋升 Fast L1；大对象 L2 命中按文件块输出，压缩对象可流式 zstd 解压，避免整文件读入内存。缓存写入使用临时文件和 rename，降低并发写同一对象造成的数据损坏风险。
...
日志通道使用非阻塞投递，避免控制面异常反压请求热路径。上传失败时会进入有限 retry buffer；批量上传按 protobuf 编码大小拆分，单个 chunk 失败时会保留当前失败日志和所有尚未发送的后续日志。

普通 TCP/UDP L4 和 `@quic` UDP 透传不生成访问日志。TCP SNI 透传会沿用对应 L7 服务的访问日志配置，并遵守全局访问日志开关；时间字段使用节点本地时区，并应用程序内部 NTP 校准偏移。
